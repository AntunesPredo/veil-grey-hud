...
export function P2PPaymentModal({ isOpen, onClose, event }: P2PPaymentModalProps) {
  ...
  const handlePay = () => {
    ...
    RetroToast.success(`CONTRIBUIÇÃO DE ${amount} ${event.payload.currency} ENVIADA!`);
    onClose();
  };

  return (
    <Modal title="CONTRIBUIR PARA FUNDO" onClose={onClose} isOpen={isOpen}>
      <div className="p-4 w-full min-w-[600px] flex flex-col gap-6">
        ...
        <div className="bg-slate-900 border-2 border-indigo-500/50 p-4 relative">
          <div className="flex justify-between items-start mb-2">
            ...
            <span className="text-indigo-400 font-mono font-bold bg-indigo-900/30 px-2 py-1">
              POOL ATUAL: {event.payload.pool} {event.payload.currency}
            </span>
          </div>
        ...
        <div className="flex flex-col gap-2">
          <h4 className="text-[var(--theme-accent)] font-bold tracking-widest text-sm mb-1">
            SELECIONE A CARTEIRA PARA CONTRIBUIR
          </h4>
          <WalletSelectorDnd
            inventory={inventory}
        ...
        {selectedWalletId && (
          <div className="bg-slate-900 border border-slate-700 p-4 animate-in fade-in slide-in-from-top-4">
            <label className="text-xs font-bold text-slate-400 block mb-2">VALOR DA CONTRIBUIÇÃO</label>
